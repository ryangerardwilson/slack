...
- Keep the scope narrow: direct message send, contact management, unread DM and mention listing, stale conversation cleanup, optional file delivery, version, and upgrade.
- Do not expand this app into a general Slack client, channel browser, or interactive TUI without explicit user direction.

## Interface Rules

- Keep the top-level interface flat: `slack ac <label> <email>`, `slack dm <contact_label|email> <message> [file_path] [dir_path]`, `slack ls -dms [-ur|-r] <number>`, `slack ls -mnts`, and `slack sc`.
