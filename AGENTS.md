...
- Keep the scope narrow: direct message send, contact management, unread DM and mention listing, optional file delivery, version, and upgrade.
- Do not expand this app into a general Slack client, channel browser, or interactive TUI without explicit user direction.

## Interface Rules

- Keep the top-level interface flat: `slack ac <label> <email>`, `slack dm <contact_label|email> <message> [file_path] [dir_path]`, and `slack ls -dm|-mnt`.
